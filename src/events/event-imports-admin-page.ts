...
<body><main class="shell">
  ...
  <section id="dashboard" class="hidden"><div class="toolbar"><div class="tabs" role="tablist"><button id="tab-events" class="tab active" role="tab">이벤트</button><button id="tab-redemption-codes" class="tab" role="tab">리딤 코드</button></div><button id="refresh">새로고침</button><span id="summary"></span></div><div class="layout">
...
export const ADMIN_IMPORTS_JS = `(() => {
  ...
  async function loadRuns() {
    const config = domainConfig(), runs = await request(config.base); showDashboard();
  ...
  async function loadRun(runId) {
    const config = domainConfig(), run = await request(config.base + '/' + encodeURIComponent(runId));
