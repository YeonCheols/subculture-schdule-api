...
운영 클라이언트 URL은 `https://subculture-game-cals.vercel.app`이다. 이 저장소의 공개 API와 수집 데이터가 클라이언트에 표시되는지 확인할 때 이 URL을 참조할 수 있으나, 클라이언트 코드·배포·설정 변경은 별도 명시 요청이 있을 때만 수행한다.
...
- 일정 데이터는 각 게임의 공식 웹사이트, 공식 포럼, 공식 라운지 또는 `config/sources.json`에 first-party 소유 근거와 immutable channel ID가 함께 등록된 공식 YouTube 채널에서만 수집한다.
- 팬사이트, 커뮤니티 추측, 유출, 검색 결과 요약을 일정 데이터로 저장하지 않는다.
- 명조 데이터는 공식 작성자 `GM 연구소` 게시물만 허용한다.
- 모든 `sourceUrl`은 원문 공식 페이지를 가리키는 HTTPS URL이어야 한다.
- 공식 출처와 작성자 검증을 통과한 수집 후보에서 일정 시각을 확인할 수 없으면 별도 후보군에 보관하거나 제외하지 않는다. 원문에서 확인한 게시 시각을 `startsAt`으로, 게시일 기준 30일 뒤 한국 날짜의 `23:59:59+09:00`을 `endsAt`으로 저장한다. 게시 시각이 없고 목록에 게시일만 있으면 그 날짜의 `00:00:00+09:00`을 추정 `startsAt`으로 사용한다. 둘 다 없을 때만 수집 시각을 사용한다. 이 구간은 원문 일정이 아니라 게시 시각·게시일·수집 시각 기준 추정값이므로 `confidence: probable`을 사용하고 `sourceTimeText`에 근거와 기간을 기록한다. 명시된 원문 시각은 언제나 이 추정값보다 우선한다.
- 원문에 종료 시각만 있으면 게시 시각, 게시일, 수집 시각 순으로 추정 `startsAt`을 선택하고 원문의 종료 시각을 `endsAt`으로 저장하며 `confidence: probable`을 사용한다. 선택한 기준이 종료 시각보다 늦으면 종료 시각을 추정 시작 시각의 상한으로 사용해 시간 순서를 보장하고, `sourceTimeText`에 추정 근거를 기록한다.
...
- 공식 YouTube 채널은 공개 RSS의 최근 최대 15개 영상으로만 발견한다. 영상 제목·설명은 일정 원문으로 사용할 수 있다. YouTube에서 확인한 코드 문자열은 출처 URL과 원문 근거를 보존해 공개 리딤코드로 저장하며, 실제 사용 가능 여부와 만료 시각을 자동으로 확정하지 않는다. 프레임, 자막, 댓글, 라이브 채팅도 수집 기능을 갖추는 경우 같은 방식으로 출처와 추출 방식을 보존한다.
...
- 날짜와 시간을 알 수 없는 공식 일정 후보는 위의 수집 기준 추정 구간을 사용하고, 종료 시각만 확인된 후보는 위의 수집 시각 기반 추정 시작값을 사용한다. 두 경우 모두 원문 시각으로 표시하지 않고 `probable`과 `sourceTimeText`로 불확실성을 표현한다.
...
- `sourceTimeText`에는 원문에서 추출한 시간 표현을 보존한다. 원문에 일정 시각이 없거나 종료 시각만 있어 수집 기준 추정값을 적용한 경우에는 그 추정 근거와 기간 또는 수집 시각을 명시한다.
...
- 관리자 화면의 리딤 코드 탭은 `GET /api/internal/admin/redemption-code-imports`에서 별도 run 상태와 배치 메타데이터를 조회한다. 성공한 임시 part는 삭제하되 `redemption-code-pages/{runId}/result.json` 결과 스냅샷은 유지하며, 미완료 part와 완료 결과는 인증된 관리자 API에서만 조회한다.
...
- 공식 원문에서 확인할 수 없는 작성자·이벤트 유형을 추측해 채우지 않는다. 일정 시각만 확인할 수 없는 공식 수집 후보에는 위의 수집 기준 30일 추정 구간을 적용하고, 원문 시각인 것처럼 표시하지 않는다.
