...
- 공식 출처와 작성자 검증을 통과한 수집 후보에서 일정 시각을 확인할 수 없으면 별도 후보군에 보관하거나 제외하지 않는다. 수집한 한국 날짜의 `00:00:00+09:00`을 `startsAt`으로, 30일 뒤 한국 날짜의 `23:59:59+09:00`을 `endsAt`으로 저장한다. 이 구간은 원문 일정이 아니라 수집 기준 추정값이므로 `confidence: probable`을 사용하고 `sourceTimeText`에 `원문에 일정 시각 없음; 수집 기준 추정 기간 (...)`을 기록한다. 명시된 원문 시각은 언제나 이 추정값보다 우선한다.
- 원문에 종료 시각만 있으면 `retrievedAt`을 추정 `startsAt`으로, 원문의 종료 시각을 `endsAt`으로 저장하고 `confidence: probable`을 사용한다. 수집 시각이 종료 시각보다 늦으면 종료 시각을 추정 시작 시각의 상한으로 사용해 시간 순서를 보장하며, 두 경우 모두 `sourceTimeText`에 추정 근거를 기록한다.
