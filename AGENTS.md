...
- 공식 YouTube 채널은 공개 RSS의 최근 최대 15개 영상으로만 발견한다. 영상 제목·설명은 일정 원문으로 사용할 수 있고, 리딤코드는 설명에 게임 맥락과 함께 명시된 공용 코드만 확정 저장한다. 프레임, 자막, 댓글, 라이브 채팅의 코드는 후보 검토용일 뿐 자동 확정하지 않는다.
