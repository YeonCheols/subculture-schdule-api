...
시간 필드는 시간대가 포함된 ISO 8601 문자열 또는 `null`이다. 공식 픽업 정보처럼 일정 시각이 없는 이벤트는 `startsAt`, `endsAt`이 모두 `null`이고 `status: unknown`일 수 있으므로 버리지 않는다. 화면 표시는 ISO 문자열의 offset을 존중하고, `sourceTimeText`는 사용자 표시용 원문 근거로 활용한다.
