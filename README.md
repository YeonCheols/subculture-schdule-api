...
| `GET` | `/api/internal/admin/event-imports/:runId/results` | 확정된 run별 페이지 manifest 조회(`ADMIN_TOKEN` 인증) |
| `GET` | `/api/internal/admin/event-imports/:runId/results/:gameId/pages/:page` | 확정된 run별 게임 페이지 JSON 조회(`ADMIN_TOKEN` 인증) |
...
├── redemption-code-pages/
│   └── {runId}/result.json             # 관리자용 완료 리딤 코드 결과 스냅샷
└── event-pages/
...
이벤트 import는 잘못된 enum, 중복 ID, timezone 없는 시각, HTTP 출처 URL을 거부합니다. 공식 수집 후보에 원문 일정 시각이 없으면 수집일 00:00 KST부터 30일 뒤 23:59:59 KST까지를 `confidence: probable`의 추정 구간으로 저장하며, `sourceTimeText`에 추정 근거를 표시합니다.
